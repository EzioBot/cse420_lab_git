...
statement : var_declaration
	  ...
	  | FOR LPAREN expression_statement expression_statement expression RPAREN statement
	  {
			...
			ForNode* forNode = new ForNode(
				(StmtNode*)$3->get_ast_node(),   // init  (ExprStmtNode*)
				(StmtNode*)$4->get_ast_node(),   // cond  (ExprStmtNode*)
				(ExprNode*)$5->get_ast_node(),   // update (ExprNode*)
				(StmtNode*)$7->get_ast_node()    // body
			);
...
variable : id_name 	
      {
		...
		else if((symtbl->Lookup_in_table($1->getname()))->getidtype() != "var")
		{
		...
		else $$->setvartype((symtbl->Lookup_in_table($1->getname()))->getvartype());
	 ...
	 | id_name LTHIRD expression RTHIRD 
	 {
		...
		else if((symtbl->Lookup_in_table($1->getname()))->getidtype() != "array")
		{
...
factor : variable
	...
	| id_name LPAREN argument_list RPAREN
	{
	    ...
	    {
	        if((symtbl->Lookup_in_table($1->getname()))->getidtype()=="func_dec")
	        {
	...
	| variable INCOP 
	{
		...
		VarNode* lhsNode  = (VarNode*)$1->get_ast_node();
		VarNode* readNode = new VarNode($1->getname(), $1->getvartype());
		ConstNode* oneNode   = new ConstNode("1", "int");
		BinaryOpNode* addNode   = new BinaryOpNode("+", readNode, oneNode, $1->getvartype());
		AssignNode* assignNode = new AssignNode(lhsNode, addNode, $1->getvartype());
		$$->set_ast_node(assignNode);
	}
	| variable DECOP
	{
		...
		AssignNode* assignNode = new AssignNode(lhsNode, subNode, $1->getvartype());
		$$->set_ast_node(assignNode);
	}
	;
