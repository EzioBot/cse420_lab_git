...
static const yytype_uint16 yyrline[] =
{
     ...
     193,   232,   249,   259,   269,   281,   294,   311,   335,   405,
     413,   421,   431,   441,   452,   460,   471,   479,   493,   507,
     513,   520,   528,   536,   544,   552,   569,   582,   596,   609,
     629,   641,   651,   663,   707,   749,   758,   798,   807,   839,
     848,   880,   890,   927,   937,  1009,  1033,  1057,  1068,  1077,
    1147,  1156,  1167,  1178,  1197,  1215,  1223,  1233,  1254
};
#endif
...
#line 74 "22301439.y"
    {
		...
		// Reuse the existing ProgramNode from $1
		ProgramNode* prog;
...
#line 193 "22301439.y"
    {
				...
				if(symtbl->Insert_in_table(func_name,"ID"))
				{
					...
					(symtbl->Lookup_in_table(func_name))->setparamlist(paramlist);
...
#line 336 "22301439.y"
    {
			...
			if((yyvsp[(1) - (3)])->getname()=="void")
			{
				...
				errors++;
				(yyvsp[(1) - (3)]) = new symbol_info("error","type");
			}
			...
			while(getline(_varlist,varname,','))
			{
				...
				else // array
				{
					...
					getline(_varname,name,'[');
					getline(_varname,size,']');
...
#line 553 "22301439.y"
    {
			...
			// FIX: $3 and $4 are expression_statement → ExprStmtNode* (a StmtNode*).
			//      $5 is a plain expression → ExprNode*.
			ForNode* forNode = new ForNode(
				(StmtNode*)(yyvsp[(3) - (7)])->get_ast_node(),   // init  (ExprStmtNode*)
				(StmtNode*)(yyvsp[(4) - (7)])->get_ast_node(),   // cond  (ExprStmtNode*)
				(ExprNode*)(yyvsp[(5) - (7)])->get_ast_node(),   // update (ExprNode*)
				(StmtNode*)(yyvsp[(7) - (7)])->get_ast_node()    // body
			);
...
#line 664 "22301439.y"
    {
		...
		else if((symtbl->Lookup_in_table((yyvsp[(1) - (1)])->getname()))->getidtype() != "var")
		{
		...
		else (yyval)->setvartype((symtbl->Lookup_in_table((yyvsp[(1) - (1)])->getname()))->getvartype());
...
#line 708 "22301439.y"
    {
		...
		else if((symtbl->Lookup_in_table((yyvsp[(1) - (4)])->getname()))->getidtype() != "array")
		{
		...
		else if((yyvsp[(3) - (4)])->getvartype()!="int")
		{
...
#line 759 "22301439.y"
    {
			...
			if((yyvsp[(1) - (3)])->getvartype() == "void" || (yyvsp[(3) - (3)])->getvartype() == "void")
			{
			...
			else if((yyvsp[(1) - (3)])->getvartype() == "int" && (yyvsp[(3) - (3)])->getvartype() == "float")
			{
			...
			if((yyvsp[(1) - (3)])->getvartype() == "error" || (yyvsp[(3) - (3)])->getvartype() == "error")
			{
				(yyval)->setvartype("error");
			}
...
#line 1078 "22301439.y"
    {
	    ...
	    {
	        if((symtbl->Lookup_in_table((yyvsp[(1) - (4)])->getname()))->getidtype()=="func_dec")
	        {
...
#line 1148 "22301439.y"
    {
		...
		(yyval)->set_ast_node((yyvsp[(2) - (3)])->get_ast_node());
	}
    break;

  case 61:
/* Line 1792 of yacc.c  */
#line 1157 "22301439.y"
    {
...
#line 1179 "22301439.y"
    {
		...
		// FIX: Create a fresh VarNode copy for use inside BinaryOpNode.
		// The original varNode becomes the lhs of AssignNode.
		// Without the copy, both AssignNode and BinaryOpNode hold the same
		// pointer → double-free when either destructor runs.
		VarNode* lhsNode  = (VarNode*)(yyvsp[(1) - (2)])->get_ast_node();
		VarNode* readNode = new VarNode((yyvsp[(1) - (2)])->getname(), (yyvsp[(1) - (2)])->getvartype());
		ConstNode* oneNode   = new ConstNode("1", "int");
		BinaryOpNode* addNode   = new BinaryOpNode("+", readNode, oneNode, (yyvsp[(1) - (2)])->getvartype());
		AssignNode* assignNode = new AssignNode(lhsNode, addNode, (yyvsp[(1) - (2)])->getvartype());
		(yyval)->set_ast_node(assignNode);
	}
    break;
...
#line 1198 "22301439.y"
    {
		...
		// FIX: Same double-free fix as INCOP above.
		VarNode* lhsNode  = (VarNode*)(yyvsp[(1) - (2)])->get_ast_node();
		VarNode* readNode = new VarNode((yyvsp[(1) - (2)])->getname(), (yyvsp[(1) - (2)])->getvartype());
		ConstNode* oneNode   = new ConstNode("1", "int");
		BinaryOpNode* subNode   = new BinaryOpNode("-", readNode, oneNode, (yyvsp[(1) - (2)])->getvartype());
		AssignNode* assignNode = new AssignNode(lhsNode, subNode, (yyvsp[(1) - (2)])->getvartype());
		(yyval)->set_ast_node(assignNode);
	}
    break;
