...
public:
    ...
    // Lookup by string name across all scopes
    symbol_info* lookup(string name)
    {
        scope_table* temp = current_scope;
        while(temp != NULL) {
            symbol_info* result = temp->lookup(name);
    ...
    // Lookup by string name in current scope only
    ...
    // Get current scope ID (for log messages)
